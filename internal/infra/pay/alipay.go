...
func VerifyAlipaySignature(values map[string][]string, publicKeyPEM string) error {
	publicKey, err := parseRSAPublicKey(publicKeyPEM)
	if err != nil {
		return fmt.Errorf("alipay callback key: %w", err)
	}
	return verifyRSASignature(publicKey, canonicalAlipayValues(values), firstHeader(values, "sign"))
}

func canonicalAlipayValues(values map[string][]string) string {
...
func (p *AlipayProvider) VerifyCallback(_ context.Context, request ProviderCallbackRequest) (ProviderCallback, error) {
	...
	amount, err := strconv.ParseFloat(firstHeader(request.Form, "total_amount"), 64)
	if err != nil {
		return ProviderCallback{}, fmt.Errorf("parse alipay amount: %w", err)
	}
	status := PaymentStateFailed
	if firstHeader(request.Form, "trade_status") == "TRADE_SUCCESS" || firstHeader(request.Form, "trade_status") == "TRADE_FINISHED" {
		status = PaymentStatePaid
	}
	paidAt, _ := time.Parse("2006-01-02 15:04:05", firstHeader(request.Form, "gmt_payment"))
	transactionID := firstHeader(request.Form, "trade_no")
	return ProviderCallback{
		OutTradeNo: firstHeader(request.Form, "out_trade_no"), TransactionID: transactionID,
