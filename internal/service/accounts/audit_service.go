...
// Export returns a CSV document. The handler owns HTTP headers and response
// writing so this service remains transport-neutral.
func (s *AuditService) Export(ctx context.Context, filter AuditFilter) ([]byte, error) {
	query := s.client.AuditLog.Query()
	applyAuditFilter(query, filter)
	...
	if err := writeAuditCSV(&output, entities); err != nil {
		return nil, err
	}
