...
func (s *RedemptionService) Delete(ctx context.Context, actorID, id string) error {
	...
	return runTransaction(ctx, s.client, func(tx *ent.Tx) error {
		...
		if deleted != 1 {
			return appRuntime.NewError(appRuntime.KindInvalidInput, "code_unavailable", "redemption code is missing or already used", appRuntime.ErrConflict)
		}
...
func (s *RedemptionService) DeleteBatch(ctx context.Context, actorID, batchID string) (int, error) {
	...
	err := runTransaction(ctx, s.client, func(tx *ent.Tx) error {
		...
		if used {
			return appRuntime.NewError(appRuntime.KindInvalidInput, "batch_contains_used_codes", "batch contains used redemption codes", appRuntime.ErrConflict)
		}
...
func (s *RedemptionService) DeleteBatchByIDs(ctx context.Context, actorID string, ids []string) (int, error) {
	...
	err := runTransaction(ctx, s.client, func(tx *ent.Tx) error {
		...
		if deleted != len(ids) {
			return appRuntime.NewError(appRuntime.KindInvalidInput, "codes_unavailable", "one or more redemption codes are missing or used", appRuntime.ErrConflict)
		}
