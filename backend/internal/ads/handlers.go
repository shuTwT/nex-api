...
func (h *Handler) list(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, r, runtime.NewValidationError(runtime.FieldError{Field: "page", Reason: "must be a positive integer"}))
		return
	}
	limit, err := positiveInt(r.URL.Query().Get("limit"), 10)
	if err != nil || limit > 100 {
		writeError(w, r, runtime.NewValidationError(runtime.FieldError{Field: "limit", Reason: "must be between 1 and 100"}))
		return
	}
	var active *bool
	if raw := strings.TrimSpace(r.URL.Query().Get("isActive")); raw != "" {
		...
		if parseErr != nil {
			writeError(w, r, runtime.NewValidationError(runtime.FieldError{Field: "isActive", Reason: "must be boolean"}))
			return
		}
		active = &value
	}
	result, err := h.service.List(r.Context(), ListOptions{Search: r.URL.Query().Get("search"), Position: r.URL.Query().Get("position"), IsActive: active, Page: page, Limit: limit})
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (h *Handler) create(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Create(r.Context(), CreateInput{Image: request.Image, ImageWidth: request.ImageWidth, ImageHeight: request.ImageHeight, Link: request.Link, Title: request.Title, Position: request.Position, IsActive: active})
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (h *Handler) update(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.service.Update(r.Context(), r.PathValue("id"), UpdateInput{Image: request.Image, ImageWidth: request.ImageWidth, ImageHeight: request.ImageHeight, Link: request.Link, Title: request.Title, Position: request.Position, IsActive: request.IsActive})
	if err != nil {
		writeError(w, r, err)
		return
	}
