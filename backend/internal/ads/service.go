...
func (s *Service) Get(ctx context.Context, id string) (*ent.Advertisement, error) {
	...
	if err != nil {
		if ent.IsNotFound(err) {
			return nil, runtime.NewAPIError(404, "not_found", "Advertisement not found", runtime.ErrNotFound)
		}
...
func (s *Service) Create(ctx context.Context, input CreateInput) (*ent.Advertisement, error) {
	...
	if exists {
		return nil, runtime.NewAPIError(409, "conflict", "该广告位已被占用", runtime.ErrConflict)
	}
	now := time.Now()
...
func (s *Service) Delete(ctx context.Context, id string) error {
	if err := s.db.Advertisement.DeleteOneID(id).Exec(ctx); err != nil {
		if ent.IsNotFound(err) {
			return runtime.NewAPIError(404, "not_found", "Advertisement not found", runtime.ErrNotFound)
		}
...
func validateCreate(input CreateInput) error {
	fields := make([]runtime.FieldError, 0, 4)
	if strings.TrimSpace(input.Image) == "" {
		fields = append(fields, runtime.FieldError{Field: "image", Reason: "required"})
	}
	if strings.TrimSpace(input.Link) == "" {
		fields = append(fields, runtime.FieldError{Field: "link", Reason: "required"})
	}
	if strings.TrimSpace(input.Title) == "" {
		fields = append(fields, runtime.FieldError{Field: "title", Reason: "required"})
	}
	if strings.TrimSpace(input.Position) == "" {
		fields = append(fields, runtime.FieldError{Field: "position", Reason: "required"})
	}
	if len(fields) > 0 {
		return runtime.NewValidationError(fields...)
	}
	return nil
}
