...
func (s *MCPService) Delete(ctx context.Context, id string) error {
	...
	if usageCount > 0 {
		return abortTx(tx, runtime.NewAPIError(409, "dependent_records", fmt.Sprintf("cannot delete MCP service %q: %d usage records still reference it", item.Name, usageCount), runtime.ErrConflict))
	}
