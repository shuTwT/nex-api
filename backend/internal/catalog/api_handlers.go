...
func (h *Handler) createAPI(w http.ResponseWriter, r *http.Request) {
	...
	input := APIInput{Name: request.Name, Alias: request.Alias, Description: request.Description, Endpoint: request.Endpoint, Method: request.Method, CategoryID: request.CategoryID, Documentation: request.Documentation, PreScript: request.PreScript, PostScript: request.PostScript, Pricing: intValue(request.Pricing), IsActive: boolValue(request.IsActive, true)}
	item, err := h.apis.Create(r.Context(), input)
	if err != nil {
		writeCatalogError(w, r, err)
		return
	}
	writeData(w, http.StatusCreated, toAPIData(item, true))
}
...
func (h *Handler) getAPI(w http.ResponseWriter, r *http.Request) {
	...
	writeData(w, http.StatusOK, toAPIData(item, true))
}
...
func (h *Handler) updateAPI(w http.ResponseWriter, r *http.Request) {
	...
	item, err := h.apis.Update(r.Context(), r.PathValue("id"), APIUpdateInput{Name: request.Name, Alias: request.Alias, Description: request.Description, Endpoint: request.Endpoint, Method: request.Method, CategoryID: request.CategoryID, Pricing: request.Pricing, Documentation: request.Documentation, PreScript: request.PreScript, PostScript: request.PostScript, IsActive: request.IsActive})
	if err != nil {
		writeCatalogError(w, r, err)
		return
	}
	writeData(w, http.StatusOK, toAPIData(item, false))
}
...
func (h *Handler) deleteAPI(w http.ResponseWriter, r *http.Request) {
	...
	writeData(w, http.StatusOK, map[string]string{"message": "API 已删除"})
}
