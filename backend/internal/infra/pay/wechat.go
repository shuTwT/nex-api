...
func VerifyWeChatSignature(body []byte, headers map[string][]string, publicKeyPEM string) error {
	timestamp := firstHeader(headers, "Wechatpay-Timestamp")
	nonce := firstHeader(headers, "Wechatpay-Nonce")
	signature := firstHeader(headers, "Wechatpay-Signature")
...
func (p *WeChatProvider) callbackVerificationKey(headers map[string][]string) (string, error) {
	serial := strings.TrimSpace(firstHeader(headers, "Wechatpay-Serial"))
