...
// QueryApiUsage chains the current query on the "apiUsage" edge.
func (_q *UserQuery) QueryApiUsage() *ApiUsageQuery {
...
// QueryApiTokens chains the current query on the "apiTokens" edge.
...
// QueryMcpUsage chains the current query on the "mcpUsage" edge.
func (_q *UserQuery) QueryMcpUsage() *McpUsageQuery {
...
// WithApiUsage tells the query-builder to eager-load the nodes that are connected to
// the "apiUsage" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithApiUsage(opts ...func(*ApiUsageQuery)) *UserQuery {
	query := (&ApiUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withApiUsage = query
	return _q
}
...
// the "apiTokens" edge. The optional arguments are used to configure the query builder of the edge.
...
// WithMcpUsage tells the query-builder to eager-load the nodes that are connected to
// the "mcpUsage" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithMcpUsage(opts ...func(*McpUsageQuery)) *UserQuery {
	query := (&McpUsageClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withMcpUsage = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withApiUsage; query != nil {
		if err := _q.loadApiUsage(ctx, query, nodes,
			func(n *User) { n.Edges.ApiUsage = []*ApiUsage{} },
			func(n *User, e *ApiUsage) { n.Edges.ApiUsage = append(n.Edges.ApiUsage, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withMcpUsage; query != nil {
		if err := _q.loadMcpUsage(ctx, query, nodes,
			func(n *User) { n.Edges.McpUsage = []*McpUsage{} },
			func(n *User, e *McpUsage) { n.Edges.McpUsage = append(n.Edges.McpUsage, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadApiUsage(ctx context.Context, query *ApiUsageQuery, nodes []*User, init func(*User), assign func(*User, *ApiUsage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ApiUsage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ApiUsageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadMcpUsage(ctx context.Context, query *McpUsageQuery, nodes []*User, init func(*User), assign func(*User, *McpUsage)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mcpusage.FieldUserId)
	}
	query.Where(predicate.McpUsage(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.McpUsageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
