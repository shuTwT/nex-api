...
func (_q *PaymentQuery) QuerySubscription() *SubscriptionQuery {
...
func (_q *PaymentQuery) WithSubscription(opts ...func(*SubscriptionQuery)) *PaymentQuery {
	query := (&SubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := _q.withSubscription; query != nil {
		if err := _q.loadSubscription(ctx, query, nodes, nil,
			func(n *Payment, e *Subscription) { n.Edges.Subscription = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PaymentQuery) loadSubscription(ctx context.Context, query *SubscriptionQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Subscription)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscription.FieldPaymentId)
	}
	query.Where(predicate.Subscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(payment.SubscriptionColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "paymentId" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
