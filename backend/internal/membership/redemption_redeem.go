...
func (s *RedemptionService) Lookup(ctx context.Context, codeInput string) (RedemptionLookup, error) {
	...
	if item.IsUsed {
		return RedemptionLookup{}, appRuntime.NewAPIError(400, "code_used", "redemption code has already been used", appRuntime.ErrConflict)
	}
	if item.ExpiresAt.Before(s.clock.Now().UTC()) && !item.ExpiresAt.IsZero() {
		return RedemptionLookup{}, appRuntime.NewAPIError(400, "code_expired", "redemption code has expired", appRuntime.ErrConflict)
	}
...
func (s *RedemptionService) Redeem(ctx context.Context, userID, codeInput string) (RedemptionResult, error) {
	...
	err := runTransaction(ctx, s.client, func(tx *ent.Tx) error {
		...
		default:
			return appRuntime.NewAPIError(400, "invalid_code_type", "redemption code type is invalid", appRuntime.ErrValidation)
		}
...
func classifyUnavailableCode(ctx context.Context, tx *ent.Tx, code string, now time.Time) error {
	...
	if item.IsUsed {
		return appRuntime.NewAPIError(400, "code_used", "redemption code has already been used", appRuntime.ErrConflict)
	}
	if !item.ExpiresAt.IsZero() && item.ExpiresAt.Before(now) {
		return appRuntime.NewAPIError(400, "code_expired", "redemption code has expired", appRuntime.ErrConflict)
	}
	return appRuntime.NewAPIError(400, "code_unavailable", "redemption code cannot be redeemed", appRuntime.ErrConflict)
}
