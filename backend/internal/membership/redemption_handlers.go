...
func (h *Handler) createCodes(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.redemption.CreateBatch(r.Context(), principal.UserID, RedemptionCreateInput{Type: body.Type, Count: count, PlanID: body.PlanID, Credits: credits, ExpiresAt: body.ExpiresAt})
	if err != nil {
		writeMembershipError(w, r, err)
		return
	}
...
func (h *Handler) redemptionPlans(w http.ResponseWriter, r *http.Request) {
	plans, err := ListRedemptionPlans(r.Context(), h.redemption.client)
	if err != nil {
		writeMembershipError(w, r, err)
		return
	}
