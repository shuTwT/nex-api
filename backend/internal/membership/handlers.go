...
func NewHandler(plans *PlanService, membership *MembershipService, redemption *RedemptionService) (*Handler, error) {
...
func RegisterServiceRoutes(mux *http.ServeMux, plans *PlanService, membership *MembershipService, redemption *RedemptionService) error {
...
func planFilter(r *http.Request) (PlanListFilter, error) {
	page, limit, err := pageQuery(r)
	if err != nil {
		return PlanListFilter{}, err
	}
	var active *bool
	if raw := r.URL.Query().Get("isActive"); raw != "" {
		value, parseErr := strconv.ParseBool(raw)
		if parseErr != nil {
			return PlanListFilter{}, appRuntime.NewValidationError(appRuntime.FieldError{Field: "isActive", Reason: "must be boolean"})
		}
		active = &value
	}
	return PlanListFilter{Search: r.URL.Query().Get("search"), IsActive: active, Page: page, Limit: limit}, nil
}

func redemptionFilter(r *http.Request) (RedemptionListFilter, error) {
	page, limit, err := pageQuery(r)
	if err != nil {
		return RedemptionListFilter{}, err
	}
	var used *bool
	if raw := r.URL.Query().Get("isUsed"); raw != "" {
		value, parseErr := strconv.ParseBool(raw)
		if parseErr != nil {
			return RedemptionListFilter{}, appRuntime.NewValidationError(appRuntime.FieldError{Field: "isUsed", Reason: "must be boolean"})
		}
		used = &value
	}
	return RedemptionListFilter{Search: r.URL.Query().Get("search"), Type: r.URL.Query().Get("type"), IsUsed: used, Page: page, Limit: limit}, nil
}
