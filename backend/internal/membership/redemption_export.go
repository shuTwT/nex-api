...
func ListRedemptionPlans(ctx context.Context, client *ent.Client) ([]*ent.SubscriptionPlan, error) {
	if err := validateContext(ctx); err != nil {
		return nil, err
	}
	if client == nil {
	...
	plans, err := client.SubscriptionPlan.Query().Where(subscriptionplan.IsActive(true)).Order(subscriptionplan.BySortOrder()).All(ctx)
	if err != nil {
