...
func NewService(database *ent.Client, manager *ScheduleManager) (*Service, error) {
	if database == nil {
...
func (s *Service) Create(ctx context.Context, input UpsertInput) (JobView, error) {
	...
	if ent.IsConstraintError(err) {
		return JobView{}, appRuntime.NewAPIError(409, "scheduled_job_exists", "a schedule for this task already exists", err)
	}
	if err != nil {
...
func (s *Service) RunNow(ctx context.Context, id string) error {
	...
	if !item.Enabled {
		return appRuntime.NewAPIError(409, "scheduled_job_disabled", "scheduled job is disabled", nil)
	}
	return s.manager.RunNow(item.ID)
}

func (s *Service) Tasks() []TaskDescriptor { return s.manager.Tasks() }

func (s *Service) validateInput(id string, input UpsertInput) (Definition, UpsertInput, error) {
	input.Name = strings.TrimSpace(input.Name)
	input.TaskKey = normalizeTaskKey(input.TaskKey)
	...
	if len(fields) > 0 {
		return Definition{}, input, appRuntime.NewValidationError(fields...)
	}
	definition := Definition{ID: id, Name: input.Name, TaskKey: input.TaskKey, ScheduleType: input.ScheduleType, Expression: input.Expression, Enabled: input.Enabled}
	if err := s.manager.Validate(definition); err != nil {
		return Definition{}, input, appRuntime.NewValidationError(appRuntime.FieldError{Field: "schedule", Reason: err.Error()})
	}
...
func definitionFromEntity(item *ent.ScheduledJob) Definition {
	return Definition{
