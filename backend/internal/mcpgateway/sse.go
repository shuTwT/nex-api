...
func (h *Handler) serveSSE(writer http.ResponseWriter, ctx context.Context, request *http.Request, service *ent.McpService, body []byte) {
	response, err := forwardStreamableHTTP(ctx, h.options.HTTPClient, service.Endpoint, request, body)
	if err != nil {
		writeError(writer, http.StatusBadGateway, "sse_upstream_failed")
		return
	}
	...
	stream := NewSSEJSONStream(ctx, response.Body, h.options.MaxResponseBytes, h.options.MaxResponseBytes)
	defer stream.Close()
	...
	if err := writeResponseBody(writer, stream, h.options.MaxResponseBytes); err != nil {
		h.logger.WarnContext(ctx, "MCP SSE stream failed", slog.String("identifier", service.Identifier), slog.Any("err", err))
	}
}
...
var _ io.ReadCloser = (*sseStream)(nil)
