...
func (h *Handler) listTokens(w http.ResponseWriter, r *http.Request) {
	...
	views, info, err := h.services.Tokens.List(r.Context(), owner.UserID, TokenFilter{Search: r.URL.Query().Get("search"), Status: r.URL.Query().Get("status")}, page)
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
...
func (h *Handler) createToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &request); err != nil {
		writeServiceError(w, r, fmt.Errorf("decode token: %w", ErrInvalidRequest))
		return
	}
