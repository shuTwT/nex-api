...
func (h *Handler) listUsers(w http.ResponseWriter, r *http.Request) {
	...
	views, info, err := h.services.Users.List(r.Context(), UserListFilter{Role: r.URL.Query().Get("role"), Search: r.URL.Query().Get("search")}, page)
	if err != nil {
		writeServiceError(w, r, err)
		return
	}
...
func (h *Handler) createUser(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &request); err != nil {
		writeServiceError(w, r, fmt.Errorf("decode user: %w", ErrInvalidRequest))
		return
	}
