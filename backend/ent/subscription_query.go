...
func (_q *SubscriptionQuery) QueryPlan() *SubscriptionPlanQuery {
...
func (_q *SubscriptionQuery) QueryPayment() *PaymentQuery {
...
func (_q *SubscriptionQuery) WithPlan(opts ...func(*SubscriptionPlanQuery)) *SubscriptionQuery {
	query := (&SubscriptionPlanClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SubscriptionQuery) WithPayment(opts ...func(*PaymentQuery)) *SubscriptionQuery {
	query := (&PaymentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := _q.withPlan; query != nil {
		if err := _q.loadPlan(ctx, query, nodes, nil,
			func(n *Subscription, e *SubscriptionPlan) { n.Edges.Plan = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPayment; query != nil {
		if err := _q.loadPayment(ctx, query, nodes, nil,
			func(n *Subscription, e *Payment) { n.Edges.Payment = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubscriptionQuery) loadPlan(ctx context.Context, query *SubscriptionPlanQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *SubscriptionPlan)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Subscription)
	for i := range nodes {
		fk := nodes[i].PlanId
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "planId" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SubscriptionQuery) loadPayment(ctx context.Context, query *PaymentQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Payment)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Subscription)
	for i := range nodes {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "paymentId" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SubscriptionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPayment != nil {
			_spec.Node.AddColumnOnce(subscription.FieldPaymentId)
		}
	}
