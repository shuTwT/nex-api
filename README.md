...
internal/    Go 应用代码
migrations/  数据库迁移
openapi/     OpenAPI 生成器与规范
frontend/    React + Vite 单页应用
```

项目根目录是 Go 模块目录；`internal` 采用五层目录（handler / infra / job / middleware / service），分层规则见 [docs/backend-architecture.md](docs/backend-architecture.md)。
...
后端配置模板在 `.env.example`。支付与 OAuth 提供商配置由管理员在系统设置中维护并保存到数据库；不要把这些凭据写入前端变量或提交到仓库。
