...
export default function ApiDetailPage() {
  ...
  const loadApiDetail = useCallback(async () => {
    if (!apiId) return;

    setIsLoading(true);
    const result = await api.marketplace_apis_id_route_get({ id: apiId });
    const data = responseData<ApiDetail>(result);
    if (data) setApiDetail(data);
    setIsLoading(false);
  }, [apiId]);
  ...
  const apiUrl = `${baseUrl}/api/v1/${apiDetail.alias}`;
  const hostName = baseUrl.replace(/^https?:\/\//, "") || "[YOUR_DOMAIN]";
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-1">
                  ...
                  <span className="font-semibold text-slate-700">{apiDetail.isActive ? "正常" : "停用"}</span>
                </div>
              </div>
              <div className="text-center">
                <div className="text-sm text-slate-500 mb-1">请求方式</div>
                <Badge className="bg-blue-100 text-blue-700 border-blue-200">{apiDetail.method}</Badge>
              </div>
              <div className="text-center">
                <div className="text-sm text-slate-500 mb-1">计费方式</div>
                <div className="flex items-center justify-center gap-1">
                  <span className="font-semibold text-slate-700">
                    {apiDetail.isFree ? "免费" : `${apiDetail.pricing} 积分/次`}
                  </span>
                </div>
              </div>
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-1">
                  ...
                  <span className="font-semibold text-slate-700">{apiDetail.todayCallCount.toLocaleString()}</span>
                </div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-6">
          <aside className="hidden md:block">
            <div className="sticky top-20 space-y-4">
              ...
              <Link to="/api-market" className="block">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Badge className="bg-blue-50 text-blue-700 border-blue-200">{apiDetail.method}</Badge>
                    <Badge className="bg-green-50 text-green-700 border-green-200">{apiDetail.category}</Badge>
                    <Badge className={apiDetail.isActive ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}>
                      {apiDetail.isActive ? "正常" : "停用"}
                    </Badge>
                    <Badge className={apiDetail.isFree ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-purple-50 text-purple-700 border-purple-200"}>
                      {apiDetail.isFree ? "免费" : "会员专享"}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-gray-50 p-4 rounded-md text-center">
                      <p className="text-2xl font-bold text-slate-900">{apiDetail.userCount}</p>
                      <p className="text-xs text-slate-500 mt-1">使用人数</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-md text-center">
                      <p className="text-2xl font-bold text-slate-900">{apiDetail.todayCallCount.toLocaleString()}</p>
                      <p className="text-xs text-slate-500 mt-1">今日调用</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-md text-center">
                      <p className="text-2xl font-bold text-slate-900">{apiDetail.totalCallCount.toLocaleString()}</p>
...
{`curl -X ${apiDetail.method} "${apiUrl}?token=YOUR_TOKEN" \
  -H "Content-Type: application/json"`}</pre>
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-900 text-white p-4 rounded-md overflow-x-auto">
                    <pre className="text-sm font-mono">
{`${apiDetail.method} ${apiDetail.endpoint}?token=YOUR_TOKEN HTTP/1.1
Host: ${hostName}
...
response = requests.${apiDetail.method.toLowerCase()}(url, params=params, headers=headers)
data = response.json()
print(data)`}</pre>
                  </div>
                </TabsContent>
