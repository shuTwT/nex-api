      ...
      const interval = setInterval(async () => {
        ...
        if (result.success && data) {
          if (data.status !== "pending") {
            ...
            if (data.status === "paid") {
              setTimeout(() => {
                navigate(`/payment/result?outTradeNo=${outTradeNo}&status=success`);
              }, 2000);
            }
          }
        }
      ...
      <div className="container px-4 py-16 md:px-6 mx-auto">
        <Card className="max-w-md mx-auto">
          <CardContent className="p-8 text-center">
            ...
            <Button onClick={() => navigate("/pricing")}>返回定价页面</Button>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="container px-4 py-16 md:px-6 mx-auto">
      <Card className="max-w-lg mx-auto">
        ...
        <CardContent className="space-y-6">
            ...
            <>
              ...
              {payment.method === "mock" && payment.payUrl && (
                <div className="text-center space-y-4">
                  ...
                  <Button
                    onClick={() => navigate(payment.payUrl ?? "/pricing")}
                    className="w-full"
                    size="lg"
                  >
                    前往模拟支付
          ...
          {(payment?.status === "failed" || payment?.status === "cancelled" || payment?.status === "expired") && (
            <div className="text-center space-y-4">
              ...
              <Button onClick={() => navigate("/pricing")}>重新选择计划</Button>
            </div>
          )}
        </CardContent>
        ...
        <CardFooter className="flex justify-center gap-4">
          ...
          {payment?.status === "pending" && (
            <Button variant="ghost" onClick={() => navigate("/console/membership")}>
              查看我的订阅
            </Button>
          )}
        </CardFooter>
