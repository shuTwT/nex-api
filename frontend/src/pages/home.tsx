...
export default function HomePage() {
  ...
  function handleSearch() {
    ...
    if (trimmed) {
      navigate(`/api-market?q=${encodeURIComponent(trimmed)}`);
    } else {
      navigate("/api-market");
    }
  }
  ...
  async function loadData() {
    ...
    const [apisResult, statsResult, announcementResult] = await Promise.all([
      ...
      api.system_settings_announcement_route_get(),
    ]);
    ...
    const announcementData = responseData<{ enabled: boolean; content: string }>(announcementResult);
    if (announcementData) {
      setAnnouncement(announcementData);
      if (announcementData.enabled && announcementData.content) {
      ...
      <section className="container px-4 py-16 md:px-6 mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {apis.slice(0, 5).map((apiItem) => (
              <div key={apiItem.id}>
                <Link to={`/api-detail?id=${apiItem.id}`}>
                    ...
                    <CardContent className="p-6 space-y-4">
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-400">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{apiItem.userCount} 人使用</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <Zap className="h-3 w-3" />
                          <span>今日：{apiItem.todayCallCount.toLocaleString()}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
