...
export default function SettingsPage() {
  ...
  async function loadSettings() {
    const result = await api.system_settings_route_get();
    const data = responseData<SystemSetting[]>(result);
    if (data) {
      ...
      try {
        if (settingsMap.oauthProviders) {
          setOauthProviders(JSON.parse(settingsMap.oauthProviders) as OAuthProvider[]);
        }
      } catch {
        // keep empty provider list on parse failure
      }
    }
    setIsLoading(false);
  }
  ...
  async function loadDefaultSettings() {
    const result = await api.system_settings_defaults_route_get();
    const data = responseData<DefaultSettings>(result);
    if (data) setDefaultSettings(data);
  }
  ...
  async function handleSave() {
    ...
    const result = await api.system_settings_route_put({ settings: settingsToUpdate });
    if (result.success) {
