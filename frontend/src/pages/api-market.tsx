...
export default function ApiMarketPage() {
  ...
  async function loadData() {
    ...
    const [apisResult, statsResult] = await Promise.all([
      api.marketplace_apis_route_get(),
      api.marketplace_stats_route_get(),
    ]);

    const apisData = responseData<MarketplaceApi[]>(apisResult);
    if (apisData) setApis(apisData);

    const statsData = responseData<MarketplaceStats>(statsResult);
  ...
  const categories = [{ name: "全部", icon: Grid3x3, count: stats?.totalApis || 0 }];

  const uniqueCategories = [...new Set(apis.map((item) => item.category))];
  uniqueCategories.forEach((cat) => {
    categories.push({
      name: cat,
      icon: Sparkles,
      count: apis.filter((item) => item.category === cat).length,
    });
  });
          ...
          <main className="space-y-6">
            ...
            {isLoading ? (
                        ...
                        <CardContent className="p-6 space-y-4">
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-400">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{item.userCount} 人使用</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <Zap className="h-3 w-3" />
                              <span>今日：{item.todayCallCount.toLocaleString()}</span>
                            </div>
                          </div>
                        </CardContent>
                      </Card>
              ...
              <div className="space-y-4">
                {sortedAPIs.map((item) => (
                  <Link key={item.id} to={`/api-detail?id=${item.id}`}>
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex flex-col items-end gap-2 text-sm text-gray-500">
                            ...
                            <div className="flex items-center gap-1">
                              <Zap className="h-3 w-3" />
                              <span>今日 {item.todayCallCount.toLocaleString()} 次</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <TrendingUp className="h-3 w-3" />
                              <span>累计 {item.totalCallCount.toLocaleString()}</span>
                            </div>
                          </div>
                        </div>
                      </CardContent>
